...
export async function listar(req, res) {
  ...
  const result = await servicosService.listarServicos(req.tenantId, { page: Math.max(1, Number(page) || 1), limit: Math.min(100, Math.max(1, Number(limit) || 20)), search: search || "" });
  res.json(result);
}
